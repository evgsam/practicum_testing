...
ostream& operator<<(ostream& out, const pair<Elements1, Elements2>& stream) {
    out <<stream.first<< ": "s<<stream.second;
    return out;
}

template <typename Elements>
ostream& Print(ostream& out, const Elements& stream) {
    bool first_elem = true;
    for (const auto& elem : stream) {
        if (first_elem) {
            first_elem = false;
            out << elem;
        }
...
template <typename Element>//, typename Element_s>
...
template <typename Element_f, typename Element_s>
ostream& operator<<(ostream& out, const map<Element_f, Element_s>& container){
    out << "{";
    Print(out, container);
    out << "}";
    return out;
...
int main() {
    {
        ...
        const set<int> expected_primes = {2, 3, 5, 7, 11, 113};
